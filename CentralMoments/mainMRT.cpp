...
const int nx = 128, ny = nx, np = 9;
const double U_ref = 2., Reynolds = 1e10, Tmax = 100., Scale_time = 1E-4;
  ...
  for(int x=0; x<nx; x++)
    for(int y=0; y<ny; y++)
      ...
      id = x*ny+y;
        ...
        temp_pop[2] = r3*0.25*cs2 - r4*0.25 - (r6 + r8)*0.5 + R*wf[1] + r2*cs2;
        temp_pop[3] = r7*0.5 - r8*0.5 + r4*0.25 + r3*0.25*cs2 + R*wf[1] - r1*cs2;
        temp_pop[4] = r6*0.5 - r8*0.5 - r4*0.25 + r3*0.25*cs2 + R*wf[1] - r2*cs2;
        temp_pop[5] = (r5 + r6 + r7 + r8)*0.25 + (r1 + r2 + r3)*0.25*cs2 + R/36.;
        temp_pop[6] = (r6 + r8)*0.25 - (r5 + r7)*0.25 + (r2 + r3)*0.25*cs2 - r1*0.25*cs2 + R*wf[8];
        temp_pop[7] = (r5 + r8)*0.25 - (r6 + r7)*0.25 + r3*0.25*cs2 - (r1 + r2)*0.25*cs2 + R*wf[8];
        temp_pop[8] = (r7 + r8)*0.25 - (r5 + r6)*0.25 + (r1 + r3)*0.25*cs2 - r2*0.25*cs2 + R*wf[8];
    	}
			for(int k=0; k<np; k++)
			{
        newx = x+cx[k];
